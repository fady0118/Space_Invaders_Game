...
unsigned long enemy_attack_timer=0;
unsigned long enemy_laser_pass=30;
...
Flag=1;		
if(special_attack_timer<41){
special_attack_timer++;}
if(enemy_attack_timer<enemy_laser_pass+1){
enemy_attack_timer++;}
}
...
const unsigned char Enemy_Lasers[] ={
 ...
 0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0xFF,
...
const unsigned char Player_Explosion[] ={
 ,
...
Stype Enemy[enemy_num]; // Enemy sprites
...
// draw the Enemy Laser
		for(unsigned long r=Enemy_Laser_number;r>0;r--){
		if(Enemy_Laser[r].life==1){
			Nokia5110_PrintBMP(Enemy_Laser[r].x, Enemy_Laser[r].y,Enemy_Laser[r].image[0] , 0);
		}
		}
	//draw the Explosion
		if(Explosion_Flag==1){
		...
		Nokia5110_PrintBMP(Player_Explosion_x, Player_Explosion_y,Player_Explosion , 0);
			Player_Explosion_Flag=0; //Acknowledge
		}
...
void Normal_Attack_update(void){
	//the update function handles moving the sprite after its initialization
	for(unsigned long r=Laser_number;r>0;r--){
	// Laser tip
...
//---------------Enemy_Attacks---------------
// it's no fun if enemies don't fight back
...
//1) pick a random number from 0 to 3 that's the index of the enemy that will attack
  ...
  n=rand()%5;
	...
	while (Enemy[n].life==0);

//Second initialize the laser
// The x-position of the enemy laser is =	enemy[i].x + Enemy_width/2 - Laser_width/2
	Enemy_Laser[Enemy_Laser_number].x = Enemy[n].x + enemy_width/2-Laser_width/2; // initial laser x-position
...
for(unsigned long r=Enemy_Laser_number;r>0;r--){
	// Laser tip
		unsigned long Lasertip_x,Lasertip_y;
		Lasertip_x=Enemy_Laser[r].x+Laser_width/2;
		Lasertip_y=Enemy_Laser[r].y;
		// is there a laser
		if(Enemy_Laser[r].life==1){
			//check if laser is out of bounds
			if(Enemy_Laser[r].y==47){
				Enemy_Laser[r].life=0;
			}
			else{
				// check if laser hits player
				//needs work
					if(player.life>0 && Lasertip_y>=(player.y-SpaceShip_height) && Lasertip_y<=player.y  && Lasertip_x>=player.x && Lasertip_x<=player.x +SpaceShip_width){
						Enemy_Laser[r].life=0;
						player.life--;
						Player_Explosion_x=player.x;
						Player_Explosion_y=player.y;
						Player_Explosion_Flag=1;
					}
...
// wait 100ms then clear display
  ...
  while(1){ 
		...
		switch(Attack_Flag){
			case 0x01:
				Laser_number= Laser_number%19 + 1; //Next laser 1,2....19,1,2...
				Normal_Attack();
		...
		if(Sound_length>5000){
			...
			Nokia5110_OutString("Press Fire1");
